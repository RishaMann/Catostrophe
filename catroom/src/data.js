/* ============================================================================
   ...
   floorCovering: предмет всегда рисуется НИЖЕ любой другой мебели и кота,
   независимо от их взаимного floorDepth (пол.x+пол.y) — ковёр лежит НА полу,
   а не «где-то по глубине среди мебели». Раньше это решалось по высоте
   (s[2] < 0.2), но тот же порог используют и низкие-но-обычные предметы
   (миски, весы, пылесос) — они должны сортироваться как всегда, а не
   провалиться под ковёр. Отдельный флаг underCovering — задел на случай,
   если когда-нибудь появится предмет, который по сюжету лежит ПОД ковром
   (спрятан) — он будет рисоваться ещё ниже floorCovering, см.
   floorRenderDepth в room/itemsRender.js.
   ========================================================================== */
(function (root) {
  'use strict';
  ...
  const ITEMS_LIST = [
    ...
    { id: 'rug',       ru: 'Ковёр',       cat: 'low',     s: [2.8, 2.4, .03], floorCovering: 1 },
