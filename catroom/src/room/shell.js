...
(function (root) {
  ...
  root.MIXIN_SHELL = {
    ...
    drawShell() {
      ...
      //
      // Блочный «пиксельный» вид вместо прежнего скруглённого прямоугольника
      // с кружком-индикатором (тот читался слишком по-векторному рядом с
      // остальной сценой — ни у фото-фона, ни у спрайтовой мебели, ни даже у
      // «пиксельных» колец света (room/lighting.js) нет ни одной скруглённой
      // грани). Плашка/тумблер строятся билинейно по 4 углам switchPoly() —
      // квада, а не осевого прямоугольника (стена в этой изометрии не
      // ортогональна экрану), поэтому «пиксели» здесь — маленькие
      // параллелограммы, повторяющие тот же наклон, что и сама стена, а не
      // отдельная развёрнутая иконка поверх неё. Тумблер — прямоугольник, не
      // круг, едет в верхнюю половину плашки при lightsOn, в нижнюю — при
      // выключенном (та же метафора, что у настоящего выключателя). ---
      const sp = this.switchPoly();
      const lerp2 = (p, q, t) => [p[0] + (q[0] - p[0]) * t, p[1] + (q[1] - p[1]) * t];
      const swPt = (u, v) => lerp2(lerp2(sp[0], sp[1], u), lerp2(sp[3], sp[2], u), v);
      const swQuad = (u0, v0, u1, v1) => [swPt(u0, v0), swPt(u1, v0), swPt(u1, v1), swPt(u0, v1)];
      // Обводка пластины — только в line-режиме (там она часть общего
      // «чертёжного» стиля сцены, все контуры в chalk/white); поверх
      // спрайтовой мебели тот же белый контур выглядит чужеродно —
      // единственная белая рамка на фотореалистичной картинке.
      poly(swQuad(0, 0, 1, 1), 0x1E1A17, 0.92, this.furnitureSprites ? null : COL.chalk, 1);
      poly(swQuad(0.14, 0.14, 0.86, 0.86), COL.chalk, 0.45, null);
      const nubV0 = this.lightsOn ? 0.5 : 0.14, nubV1 = this.lightsOn ? 0.86 : 0.5;
      // «Выключено» — почти сливается с тёмной пластиной (не тлеет), а не
      // средне-серый: на тёмной сцене средне-серый на вид ярче, чем тусклый
      // амбер на УЖЕ подсвеченном (когда свет включён) фоне — контраст
      // обманывал, состояние читалось наоборот. «Включено» — не просто
      // плоская заливка, а маленький ЛИЧНЫЙ блик светлее самого амбера
      // (не связан с room/lighting.js — это индикатор, не источник света
      // комнаты): однозначно «горит» при любом освещении сцены.
      poly(swQuad(0.28, nubV0 + 0.06, 0.72, nubV1 - 0.06), this.lightsOn ? COL.amber : 0x2E2A26, 1, null);
      if (this.lightsOn) {
        const midV = (nubV0 + nubV1) / 2;
        poly(swQuad(0.42, midV - 0.09, 0.58, midV + 0.09), 0xFFEFC2, 1, null);
      }

      // Точка крепления лампы/люстры (lightPoly, двигается как дверь/окно —
      // hitLight/dragOpening) теперь рисуется в drawCeilInto()
      // (room/itemsRender.js, на графике самого потолочного предмета,
      // CEIL_DEPTH = 900), не тут: на gShell (SHELL_DEPTH = -1) она
      // пряталась под мебелью на полу — а крепление на потолке должно быть
      // видно поверх всего в комнате.
