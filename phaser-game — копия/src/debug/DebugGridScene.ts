...
export class DebugGridScene extends Phaser.Scene {
  ...
  private applyPersistentSettings() {
    ...
    room.cameras.main.setZoom(this.settings.zoom);
    this.cameras.main.setZoom(this.settings.zoom); // держим оверлеи (сетка, панель) в одном масштабе с комнатой
  }

  private createSettingsUI() {
    const P = DebugGridScene.PANEL;
    this.settingsG = this.add.graphics().setDepth(999);
    this.settingsText = this.add.text(P.x + 8, P.y + 6, "", { fontSize: "7px", color: "#ebe2d5" }).setDepth(1000);
    ...
    this.input.on("pointermove", (p: Phaser.Input.Pointer) => {
      ...
      const f = clamp((p.x - DebugGridScene.TRACK_X0) / (DebugGridScene.TRACK_X1 - DebugGridScene.TRACK_X0), 0, 1);
  ...
  private drawToggleBtn(
    ...
    if (!t) {
      t = this.add.text(0, 0, "", { fontSize: "6px" }).setOrigin(0.5).setDepth(1002);
  ...
  private addTinyText(x: number, y: number, text: string, color: string) {
    let t = this.labelPool[this.labelsUsed];
    if (!t) {
      t = this.add.text(0, 0, "", { fontSize: "6px" }).setOrigin(0.5).setDepth(1000);
      this.labelPool.push(t);
    }
